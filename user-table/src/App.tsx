...
export default function App() {
  ...
  const cities = Array.from(new Set(users.map((user) => user.address.city))).sort((left, right) =>
    left.localeCompare(right),
  );

  useEffect(() => {
    if (selectedCity !== "all" && cities.length > 0 && !cities.includes(selectedCity)) {
      setSelectedCity("all");
    }
  }, [cities, selectedCity]);
  ...
  return (
    <main className="min-h-screen px-4 py-8 text-foreground sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <section className="rounded-[24px] border border-white/70 bg-surface shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
          <div className="border-b border-[#e8edf5] px-5 py-5 sm:px-6">
            ...
            <p className="mt-2 text-sm text-[#64748b]">
              Search by name or email, filter by city, and sort by name from the table header.
            </p>
          </div>

          <div className="flex flex-col gap-4 px-5 py-5 sm:px-6 md:flex-row md:items-end md:justify-between">
            <div className="w-full md:max-w-sm">
              <label htmlFor="search" className="mb-2 block text-sm font-medium text-[#334155]">
                Search
              </label>
              <input
                id="search"
                ...
                className="h-11 w-full rounded-xl border border-border bg-white px-4 outline-none transition focus:border-[#93c5fd] focus:ring-4 focus:ring-[#dbeafe]"
              />
            </div>

            <div className="w-full md:max-w-xs">
              <label htmlFor="city" className="mb-2 block text-sm font-medium text-[#334155]">
                Filter by city
              </label>
              <select
                id="city"
                ...
                <table className="min-w-full border-collapse">
                  <thead className="bg-[#f8fafc]">
                    <tr>
                      <SortableHeader
                        ...
                        onClick={() => setSortDirection((current) => getNextSortDirection(current))}
                      >
                        Name
                      </SortableHeader>
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      <TableMessage colSpan={4}>Unable to load users: {error}</TableMessage>
                    ) : filteredUsers.length === 0 ? (
                      <TableMessage colSpan={4}>No users match the current search and filter.</TableMessage>
                    ) : (
                      filteredUsers.map((user) => (
                        <tr key={user.id} className="border-t border-[#eef2f7]">
                          ...
                          <td className="px-5 py-4 text-sm text-[#475569]">{user.email}</td>
                          <td className="px-5 py-4 text-sm text-[#475569]">{user.company.name}</td>
                          <td className="px-5 py-4 text-sm text-[#475569]">{user.address.city}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
...
function SortableHeader({
  ...
  const indicator = activeDirection === "desc" ? "↓" : activeDirection === "asc" ? "↑" : "↕";
  ...
  return (
    <th className="px-5 py-4 text-left text-sm font-semibold text-[#334155]">
      <button
        type="button"
        onClick={onClick}
        className="inline-flex items-center gap-2 rounded-md text-inherit transition hover:text-[#1d4ed8]"
      >
        <span>{children}</span>
...
function TableMessage({
  ...
  return (
    <tr className="border-t border-[#eef2f7]">
      <td colSpan={colSpan} className="px-5 py-10 text-center text-sm text-[#64748b]">
        {children}
      </td>
    </tr>
  );
